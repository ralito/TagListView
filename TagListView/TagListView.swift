...
@objc public protocol TagListViewDelegate {
	...
	@objc optional func tagAddButtonPressed(_ title: String, tagView: TagView, sender: TagListView) -> Void
}
...
open class TagListView: UIView {
    ...
    @IBInspectable open dynamic var paddingY: CGFloat = 14 {
        didSet {
            for tagView in tagViews {
    ...
    @IBInspectable open dynamic var paddingX: CGFloat = 20 {
        didSet {
            for tagView in tagViews {
    ...
    @IBInspectable open dynamic var marginX: CGFloat = 10 {
        didSet {
            rearrangeViews()
        }
    }
    ...
    func removeButtonPressed(_ closeButton: CloseButton!) {
        if let tagView = closeButton.tagView {
            switch tagView.removeButtonType {
            case .Add:
                delegate?.tagAddButtonPressed?(tagView.currentTitle ?? "", tagView: tagView, sender: self)
            case .Remove:
